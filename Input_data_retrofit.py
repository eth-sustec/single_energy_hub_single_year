...
Demands = Demands.stack().stack().reorder_levels([3, 2, 0, 1]).to_dict()
...
P_solar = pd.read_excel(
    "Time_series_inputs_retrofit.xlsx", index_col=[0, 1], header=[0], sheet_name="Solar"
)  # Read from some Excel/.csv file
P_solar = P_solar.stack().reorder_levels([2, 0, 1]).to_dict()
